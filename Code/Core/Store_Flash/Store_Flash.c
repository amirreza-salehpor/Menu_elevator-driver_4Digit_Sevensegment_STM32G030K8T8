...
//---------------------------------------------------- 
#define Low_speed_D 12
#define Lock_Distance_D 6
#define Accel_Lock_D 6


uint8_t Obstacle_KP_D[1]=                {20};                  															//OBP
uint8_t Learnig_Speed_D[1]=              {10};                          											//LSP
uint8_t Open_Accel_D[6]=                 {4,Accel_Lock_D,Accel_Lock_D,6,4,4};           	    //OPA
uint8_t Open_Speed_D[6]=                 {8,Low_speed_D,6,60,10,5};      										  //OPS
uint8_t Open_Current_D[6]=               {100,100,100,100,100,100};   												//OPC
uint8_t Open_Position_D[6]=              {0,1,4,Lock_Distance_D,50,98};     									//OPP
uint8_t Close_Accel_D[6]=                {4,10,4,Accel_Lock_D,Accel_Lock_D,Accel_Lock_D};     //CLA
uint8_t Close_Speed_D[6]=                {7,60,6,Low_speed_D,5,1};        										//CLS
uint8_t Close_Current_D[6]=              {100,100,100,100,100,100};   												//CLC
...
uint8_t KP_Detection_Time_D[1]=          {10};                  															//OBD
uint8_t KP_Speed_Constant_D[1]=          {50};                 															  //OPC
uint8_t Opening_Destination_D[1]=        {11};                 													  		//DLO
uint8_t Closing_Destination_D[1]=        {8};                 														  	//DLC
uint8_t Normal_Jerk_D[1]=                {6};                  															  //NOJ
uint8_t Stop_Jerk_D[1]=                  {40};                   															//RDJ
uint8_t Over_Heat_Limit_D[1]=            {70};                  															//OHL
...
uint8_t Open_Accel_C[6]=                 {4,Accel_Lock_D,Accel_Lock_D,6,4,4};           	    //OPA
uint8_t Open_Speed_C[6]=                 {8,Low_speed_D,6,60,10,5};      										  //OPS
uint8_t Open_Current_C[6]=               {100,100,100,100,100,100};   												//OPC
uint8_t Open_Position_C[6]=              {0,1,4,Lock_Distance_D,50,98};     									//OPP
uint8_t Close_Accel_C[6]=                {4,10,4,Accel_Lock_D,Accel_Lock_D,Accel_Lock_D};     //CLA
uint8_t Close_Speed_C[6]=                {7,60,6,Low_speed_D,5,1};        										//CLS
uint8_t Close_Current_C[6]=              {100,100,100,100,100,100};   												//CLC
...
uint8_t KP_Detection_Time_C[1]=          {10};                  															//OBD
uint8_t KP_Speed_Constant_C[1]=          {50};                 															  //OPC
uint8_t Opening_Destination_C[1]=        {11};                 													  		//DLO
uint8_t Closing_Destination_C[1]=        {8};                 														  	//DLC
uint8_t Normal_Jerk_C[1]=                {6};                  															  //NOJ
uint8_t Stop_Jerk_C[1]=                  {40};                   															//RDJ
uint8_t Over_Heat_Limit_C[1]=            {70};                  															//OHL
...
uint8_t Command_Source_Mode_C[1]=        {1};                   															//CSR	
uint8_t Demo_C[1]=                       {0};                   															//DENO
//-----------------------------------------
uint8_t Device_Mode_C[1]=                {0};
//-----------------------------------------
...
#define Low_speed_H 15
#define Lock_Distance_H 6
#define Accel_Lock_H 6


uint8_t Obstacle_KP_H[1]=                {21};                  															//OBP
uint8_t Learnig_Speed_H[1]=              {15};                          											//LSP
uint8_t Open_Accel_H[6]=                 {4,Accel_Lock_D,Accel_Lock_D,10,10,10};           	  //OPA
uint8_t Open_Speed_H[6]=                 {8,Low_speed_D,8,90,25,8};      										  //OPS
uint8_t Open_Current_H[6]=               {100,100,100,100,100,100};   												//OPC
uint8_t Open_Position_H[6]=              {0,3,6,Lock_Distance_D,68,95};     									//OPP
uint8_t Close_Accel_H[6]=                {4,10,6,Accel_Lock_D,Accel_Lock_D,Accel_Lock_D};     //CLA
uint8_t Close_Speed_H[6]=                {12,90,8,Low_speed_D,8,8};        										//CLS
uint8_t Close_Current_H[6]=              {100,100,100,100,100,100};   												//CLC
uint8_t Close_Position_H[6]=             {100,98,55,Lock_Distance_D,3,1};   									//CLP
uint8_t Stop_Accel_H[6]=                 {10,10,10,10,10,10};  				  											//BRA
...
uint8_t KP_Detection_Time_H[1]=          {10};                  															//OBD
uint8_t KP_Speed_Constant_H[1]=          {50};                 															  //OPC
uint8_t Opening_Destination_H[1]=        {100};                 															//DLO
uint8_t Closing_Destination_H[1]=        {100};                 															//DLC
uint8_t Normal_Jerk_H[1]=                {15};                  															//NOJ
uint8_t Stop_Jerk_H[1]=                  {10};                   															//RDJ
uint8_t Over_Heat_Limit_H[1]=            {50};                  															//OHL
...
uint8_t Motor_Nominal_Speed_H[1]=        {80};                 															  //    *5
...
uint8_t Obstacle_KP_O[1]=                {22};                  															//OBP
uint8_t Learnig_Speed_O[1]=              {15};                          											//LSP
uint8_t Open_Accel_O[6]=                 {4,Accel_Lock_D,Accel_Lock_D,10,10,10};           	  //OPA
uint8_t Open_Speed_O[6]=                 {8,Low_speed_D,8,90,25,8};      										  //OPS
uint8_t Open_Current_O[6]=               {100,100,100,100,100,100};   												//OPC
uint8_t Open_Position_O[6]=              {0,3,6,Lock_Distance_D,68,95};     									//OPP
uint8_t Close_Accel_O[6]=                {4,10,6,Accel_Lock_D,Accel_Lock_D,Accel_Lock_D};     //CLA
uint8_t Close_Speed_O[6]=                {12,90,8,Low_speed_D,8,8};        										//CLS
uint8_t Close_Current_O[6]=              {100,100,100,100,100,100};   												//CLC
uint8_t Close_Position_O[6]=             {100,98,55,Lock_Distance_D,3,1};   									//CLP
...
uint8_t Over_Heat_Limit_O[1]=            {50};                  															//OHL
...
uint8_t Motor_Nominal_Speed_O[1]=        {80};                 															  //    *5
